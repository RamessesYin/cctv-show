...
(function() {
  ...
  define(["d3", "utils", "pubsub", "d3_compat", "common", "popup","application"], function(d3, utils, PubSub, d3_compat, common, popup) {
    return function() {
      ...
      PubSub.subscribe("window:resize", function(msg, _arg) {
        ...
        console.log(utils.axis.x.range()[1] - utils.axis.x.range()[0]);
        svg.select("defs #graphs_clip_path rect").attr("x", utils.axis.x.range()[0]).attr("y", 0).attr("width", 800).attr("height", height);
        svg.selectAll("#playground_bg, #playground").attr("x", utils.axis.x.range()[0]).attr("y", utils.axis.y.range()[1]).attr("width", 800).attr("height", utils.axis.y.range()[0] - utils.axis.y.range()[1]);
